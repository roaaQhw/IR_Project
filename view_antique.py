...
#         top_documents, cosine_similarities = searchQuery(result_query, matrix, vector, dataSet)
#         # Convert top_documents to JSON format
#         top_documents_json = top_documents.to_json(orient='records', force_ascii=False)
#         return jsonify({"top_documents": top_documents_json, "cosine_similarities": cosine_similarities.tolist()})
...
if __name__ == '__main__':
    ...
    # Load dataset and models
    dataset_path = r'C:\Users\hp\Desktop\IR_project_spyder\antique\collection.tsv'
    dataSet = loadData(dataset_path)
    dataSet.dropna(subset=['text'], inplace=True)
    ...
    # Load the saved TF-IDF matrix and vectorizer
    with open(r'C:\Users\hp\Desktop\IR_project\antique\tfidf_matrix.pkl', 'rb') as file:
        matrix = pickle.load(file)

    with open(r'C:\Users\hp\Desktop\IR_project\antique\vectorizer.pkl', 'rb') as file:
        vector = pickle.load(file)
